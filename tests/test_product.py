...
def test_get_public_vendor_products(client, mock_vendor_data, mock_vendor_token_data, products_data_inject, roles_data_inject, category_data_inject, approved_vendor_profile_inject):
    ...
    assert len(products.json["products"][0]) == 16


def test_get_public_vendor_products_business_name_only(
    client,
    mock_vendor_data,
    mock_vendor_token_data,
    products_data_inject,
    ...
    products = client.get(
        "/products/public-vendor-products?business_name=Eco-Foods",
        headers=mock_vendor_token_data,
    )

    assert products.status_code == 200
    assert products.json["success"] is True
    assert len(products.json["products"]) == 2
    assert len(products.json["products"][0]) == 16


def test_get_public_vendor_products_category_name_only(
    client,
    mock_vendor_data,
    mock_vendor_token_data,
    products_data_inject,
    ...
    products = client.get(
        "/products/public-vendor-products?category_name=category1",
        headers=mock_vendor_token_data,
    )
...
def test_get_public_vendor_products_repo_error(
    client,
    mock_vendor_data,
    mock_vendor_token_data,
    products_data_inject,
    ...
    assert products.json["success"] is False
    assert products.json["location"] == "view get public vendor products repo"



# repo\product.py              102      8    92%   244 - 247 (done in vendor), 251 - 257 (done in order)

# router\product.py             51      0   100%

# schemas\product.py           288      0   100%

# views\product.py             135     11    92%   83-85, 155-157, 422-423, 487-489 (exception errors)